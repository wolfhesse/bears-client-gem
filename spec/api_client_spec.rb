...
describe SwaggerClient::ApiClient do
  ...
  describe "#deserialize" do
    ...
    it "handles Hash<String, Bear>" do
      api_client = SwaggerClient::ApiClient.new
      headers = {'Content-Type' => 'application/json'}
      response = double('response', headers: headers, body: '{"bear": {"id": 1}}')
      data = api_client.deserialize(response, 'Hash<String, Bear>')
      expect(data).to be_instance_of(Hash)
      expect(data.keys).to eq([:bear])

      bear = data[:bear]
      expect(bear).to be_instance_of(SwaggerClient::Bear)
      expect(bear.id).to eq(1)
    end

    it "handles Hash<String, Hash<String, Bear>>" do
      api_client = SwaggerClient::ApiClient.new
      headers = {'Content-Type' => 'application/json'}
      response = double('response', headers: headers, body: '{"data": {"bear": {"id": 1}}}')
      result = api_client.deserialize(response, 'Hash<String, Hash<String, Bear>>')
      expect(result).to be_instance_of(Hash)
      expect(result.keys).to match_array([:data])

      data = result[:data]
      expect(data).to be_instance_of(Hash)
      expect(data.keys).to match_array([:bear])
  ...
  describe "#object_to_hash" do
    it "ignores nils and includes empty arrays" do
      ...
      expect(api_client.object_to_hash(bear)).to eq(expected)
    end
  end
